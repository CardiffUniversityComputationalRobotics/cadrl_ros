...
from cadrl_ros.network import Config
...
def find_nearest(array, value):
    ...
    tiled_value = np.tile(
        np.expand_dims(value, axis=0).transpose(), (1, np.shape(array)[0])
    )
...
def convert_cadrl_state_to_state(cadrl_state):
    ...
    if Config.MAX_NUM_OTHER_AGENTS_OBSERVED in [3, 4]:
        ...
        agent_state[:, Config.FIRST_STATE_INDEX + 0] = cadrl_state[
            :, 0
        ]  # host agent distance to goal
        agent_state[:, Config.FIRST_STATE_INDEX + 1] = cadrl_state[
            :, 3
        ]  # host agent heading in ego frame
        agent_state[:, Config.FIRST_STATE_INDEX + 2] = cadrl_state[
            :, 1
        ]  # host agent pref_speed
        agent_state[:, Config.FIRST_STATE_INDEX + 3] = cadrl_state[
            :, 6
        ]  # host agent radius
        ...
        for i in range(Config.MAX_NUM_OTHER_AGENTS_OBSERVED):
            ...
            num_agents_on[
                is_on_inds
            ] += 1  # keep track of how many agents are "on", for the RNN
        ...
        agent_state[:, 4:9] = cadrl_state[
            :, 9:14
        ]  # other agent px, py, vx, vy in body frame, radius
...
def plot_current_state_ego_frame(state, figure_name=None, axes=None):
    ...
    circ1 = plt.Circle(
        (-host_dist_to_goal, 0.0), radius=host_radius, fc="w", ec=plt_colors_local[0]
    )
    ax.add_patch(circ1)
    # goal
    ...
    wedge = ptch.Wedge(
        [-host_dist_to_goal, 0.0],
        1.0,
        rad2deg(host_heading - np.pi / 3),
        rad2deg(host_heading + np.pi / 3),
        alpha=0.1,
    )
    ax.add_patch(wedge)
    heading = ax.plot(
        [-host_dist_to_goal, -host_dist_to_goal + np.cos(host_heading)],
        [0.0, np.sin(host_heading)],
        "k--",
    )
    ...
    for i in range(len(other_pxs)):  # plot all agents that are "ON"
        ...
        circ = plt.Circle(
            (-host_dist_to_goal + other_pxs[i], other_pys[i]),
...
def plot_snapshot(
    ...
    plot_x = (
        host_pref_speed
        * possible_actions[:, 0]
        * np.cos(possible_actions[:, 1] + host_heading)
    )
    plot_y = (
        host_pref_speed
        * possible_actions[:, 0]
        * np.sin(possible_actions[:, 1] + host_heading)
    )
    plot_z = np.squeeze(probs)

    # Add dashed line between (0,0) and max speed forward
    plt.plot(
        [0.0, host_pref_speed * np.cos(host_heading)],
        [0.0, host_pref_speed * np.sin(host_heading)],
        "k--",
    )
